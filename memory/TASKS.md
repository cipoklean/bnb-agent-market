...
- [x] Phase 5: Production directory (2026-08-11) — purge mock agents, live marketplace. DONE (details: docs/submission/evidence/production-directory.md; tests/verify-live-directory.mjs 19/19).
  - [x] **LIVE directory**: lib/scan-server.ts listAgents({chainId=56, limit=24}) on the 8004scan public API (IP-rotation-safe; 60s in-memory TTL; NEVER throws → degraded flag); marketplace/alphadesk/taskchain/home = force-dynamic server components; MarketClient (client) merges store.submittedAgents; ScanAgentCard shows ONLY real fields (name or "Agent #tokenId", tokenId, owner truncated, total_score, total_feedbacks, health_score, x402_supported, is_verified badge, View on 8004scan; null metrics → "Fresh agent — no feedback yet."); filters All/x402/Verified; sort score/feedbacks; degraded banner + local submissions + CTA.
  - [x] **Mock purge**: AGENTS/getAgent → SAMPLE_AGENTS/sampleAgentById behind NEXT_PUBLIC_SAMPLE_DATA=1 (default OFF); old marketplace/home preserved as Sample* components; profile hides invented stats/chart for live agents (real indexer tiles instead); erc8004 adapter operates on samples only.
  - [x] **Home stats real**: "252,9xx agents indexed on BSC · N listed in this directory" (real meta.pagination.total); featured = top 3 by total_feedbacks.
  - [x] **Hire real agents**: scan slug / canonical id resolution via proxy; generic capability "Custom session task — you set the terms" + user scope textarea + fee cap (step 2) as payment amount; manifest task_type custom_session; honest execution note; Agent-not-found state instead of AGENTS[0] fallback.
  - [x] **Label matrix (STEP 7)**: ERC-8004 KNOWN · 8004scan LIVE · PancakeSwap addresses KNOWN/exec DEMO · x402 schema KNOWN (B402)/settlement DEMO · Altana SDK+KeyStore KNOWN/integration DEMO — in adapters, /api/evidence, settings, footer.
  - [x] Browser walk: live cards w/ real scores, submit→merge (25 listed), degraded banner observed live, scan profile + hire end-to-end (ses-5667d47e custom_session v2), zero JS errors; typecheck clean; build 14/14 (marketplace/verticals now ƒ).
- [x] Phase 6: Directory resilience (2026-08-11) — live → lastGood → snapshot → degraded; never "0 + banner" after a good result. DONE (evidence appendix in docs/submission/evidence/production-directory.md; tests/verify-directory-resilience.mjs 21/21).
  - [x] **Shared cache**: lib/directory-cache.ts getDirectory() (server-only) — 5-min TTL, module-level lastGood; on live failure serve lastGood (stale) → bundled snapshot (stale) → degraded; home/marketplace/alphadesk/taskchain all call it (no direct listAgents).
  - [x] **Snapshot**: scripts/snapshot-directory.mjs (zero-dep, IP-probe reuse shape) writes apps/web/src/lib/directory-snapshot.json (top-24 + total + fetchedAt), atomic write, old file kept on failure — build never depends on network; file committed.
  - [x] **Honest captions**: live → green dot; stale → amber "Showing cached directory — fetched {timeAgo}. Live scores return when 8004scan responds."; degraded → existing banner + empty + CTA.
  - [x] **Dev/test flags**: SCAN_FORCE_FAIL=1 (listAgents degrades), DIRECTORY_TTL_MS, DIRECTORY_DISABLE_SNAPSHOT=1; tests/json-loader.mjs resolve hook (extensionless imports retry as .ts) + json load hook — lib modules now node-testable.
  - [x] Walked: SCAN_FORCE_FAIL=1 → full 24-agent snapshot + amber caption on marketplace + home, zero JS errors; test C proves live→lastGood stale invariant against the real network; real-outage behavior observed on this box (DNS round-robins the blackholed IP). typecheck clean; build 14/14 (snapshot bundled); all suites PASS (19/19, 21/21, 29/29 offline gate + checksum).
